...
-- DUAL(DUMMY) 테이블 : 가짜 테이블(임시 조회용 테이블)

-- 날짜 + 산술 연산(+, -)
SELECT SYSDATE - 1, SYSDATE, SYSDATE + 1 FROM DUAL;
-- 날짜에 +/- 연산 시 일 단위로 계산 진행

/* <컬럼 별칭 지정>
 * SELECT 조회 결과의 집합인 RESULT SET에 출력되는 컬럼명을 지정
 * 
 * 컬럼명 AS 별칭 : 별칭 띄어쓰기X, 특수문자X, 문자만 O
 * 컬럼명 AS "별칭" : 별칭 띄어쓰기O, 특수문자O, 문자만 X
 * 
 * AS 생략 가능
 */
...
-- 2) DISTINCT 구문은 SELECT 제일 앞에 작성되어야 함
...
-- EAMIL에서 _ 앞에 글자가 세 글자인 사원 조회
SELECT EMP_NAME, EMAL
FROM EMPLOYEE
WHERE EMAIL LIKE '____%';

-- 문제점
-- _를 기준으로 삼았으나 와일드 카드 '_'와 동일한 표기법으로 작성되어 구분되지 않음

-- 해결 방법
-- LIKE의 ESCAPE OPTION을 이용하여 _를 구분

-- LIKE의 ESCAPE OPTION
-- 일반 문자로 처리할 '_' / '%' 앞에 아무 특수기호를 첨부하여 구분
SELECT EMP_NAME, EMAL
FROM EMPLOYEE
...
/* NULL 처리 연산자
 * JAVA NULL : 참조하는 객체가 없음을 의미
 * DB   NULL : 컬럼에 값이 없음을 의미
 * 
 * 1) IS NULL : NULL인 경우 조회
 * 2) IN NOT NULL : NULL이 아닌 경우 조회
 */
