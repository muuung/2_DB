...
-- EMPLOYEE 테이블에서 부서 별로 같은 직급인 사원의 수를 조회
-- 부서코드 오름차순, 직급코드 내림차순
...
-- EMPLOYEE 테이블에서 직급 별 인원 수가 5명 이하인 직급코드, 인원 수 조회
-- 직급코드 오름차순
SELECT JOB_CODE, COUNT(*)
FROM EMPLOYEE
GROUP BY JOB_CODE 
HAVING COUNT(*) <= 5 -- HAVAING절에는 그룹 함수가 반드시 작성됨
ORDER BY JOB_CODE;
 ...
 * UNION ALL : UNION + INTERSECT(합집합에서 중복 부분 제거X)
...
-- 조회하는 컬럼의 이름, 개수가 모두 동일해야함

-- 서로 다른 테이블이지만 컬럼의 타입, 개수만 일치하면 집합 연산자 사용 가능
SELECT EMP_ID, EMP_NAME FORM EMPLOYEE
UNION
