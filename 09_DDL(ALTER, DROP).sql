...
-- 객체를 만들고(CREATE), 바꾸고(ALTER), 삭제(DORP)하는 데이터 정의 언어
...
-- 컬럼 삭제 시 주의사항!
-- 테이블 : 행과 열로 이루어진 DB의 가장 기본적인 객체로, 테이블에 데이터가 저장됨
...
('D10', '개발1팀', 'L1', DEFAULT, DEFAULT);
-- "KH_KHJ"."DEPT_COPY"."DEPT_ID" 열에 대한 값이 너무 큼(실제: 3, 최대값: 2)
...
ALTER TABLE DEPT_COPY RENAME COLUMN DEPT_TITLE TO DPET_NAME;

SELECT * FROM DEPT_COPY;

-- 제약조건명 변경(D_COPY_PK -> DEPT_COPY_PK)
ALTER TABLE DEPT_COPY RENAME CONSTRAINT D_COPY_PK TO DEPT_COPY_PK;

-- 테이블명 변경(DEPT_COPY -> DCOPY)
